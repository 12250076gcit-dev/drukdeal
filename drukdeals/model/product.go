...
func (p *Product) Create() error {
	query := `INSERT INTO products (user_id, cat_id, title, price, description, image_path)
	          VALUES ($1, $2, $3, $4, $5, $6) RETURNING prod_id`
	return db.DB.QueryRow(query, p.UserID, p.CatID, p.Title, p.Price, p.Description, p.ImagePath).Scan(&p.ProdID)
}
...
func (p *Product) Delete() error {
	query := "DELETE FROM products WHERE prod_id = $1 AND user_id = $2"
